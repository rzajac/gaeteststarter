...
class BaseTestCase(unittest.TestCase):
    ...
    def assertRedirects(self, response, to=None):
        ...
        if to is not None:
            error = 'Response redirected, but went to %s instead of %s' % (response.location, to)
    ...
    def get_tasks(self, url=None, name=None, queue_names=None):
        ...
        for task in tasks:
            ...
            if task.get('eta'):
                task['eta_datetime'] = datetime.datetime.strptime(task['eta'], self.TASK_ETA_FORMAT)
    ...
    def check_if_api_error(self, response):
        ...
        self.assertTrue(response['status_code'] == 400 or response['status_code'] == 401, 'API status code should be 400 or 401.')
